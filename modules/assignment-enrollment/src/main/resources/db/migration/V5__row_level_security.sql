...
create policy tenant_isolation on enrollment_events
...
alter table enrollment_projection enable row level security;
alter table enrollment_projection force row level security;

create policy tenant_isolation on enrollment_projection
    using (tenant_id = current_setting('app.tenant_id', true) or current_setting('app.tenant_id', true) = '*')
    with check (tenant_id = current_setting('app.tenant_id', true) or current_setting('app.tenant_id', true) = '*');
